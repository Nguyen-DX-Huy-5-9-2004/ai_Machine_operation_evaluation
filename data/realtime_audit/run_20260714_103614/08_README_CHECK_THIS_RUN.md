...
```bash
G:\My Drive\OBAD\inference\online\score_new_events.py --config inference/online/config.local.yaml --stage-only --audit --max-events 100 --candidate-mode historical-overlap
```
...
- Raw candidate rows: 13
- Raw context rows: 532
- Processed rows: 13
- Closed rows chuẩn bị score: 13
- Open/skipped rows: 0
- Historical compare available: True
- Historical compare source: sql_table
- Historical compare error: None
- L1 mode: disabled_noop
- L2 mode: not_run
- Write SQL enabled: False
- Technical result: PASS
- Model readiness result: FAIL_HISTORICAL_COMPARE_MISMATCH
- Result: FAIL
