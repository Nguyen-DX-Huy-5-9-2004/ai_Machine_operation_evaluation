...
export default function App() {
  ...
  const [modelMonitorStatus, setModelMonitorStatus] = useState<{ data: ModelMonitorDto | null; loading: boolean; error: string | null }>({ data: null, loading: false, error: null });
  const [filters, setFilters] = useState<RuntimeFilters>({ datasetMode: DEFAULT_DATASET_MODE, rangePreset: 'Last 30 Days' });
  ...
  useEffect(() => {
    ...
    dataProvider.modelMonitorDto(filters, controller.signal)
      .then((monitor) => setModelMonitorStatus({ data: monitor, loading: false, error: null }))
      .catch((reason: Error) => {
  ...
  else if (activePage === 'machines') content = <RuntimeMachinesPage filters={filters} onSelect={(machineId) => { setSelectedMachine(machineId); window.history.pushState({}, '', `/machine-detail?machineId=${machineId}`); }} />;
  ...
  else content = data ? <DashboardPage data={data} loading={loading} rangePreset={filters.rangePreset} onRangePresetChange={(rangePreset) => setFilters((current) => ({ ...current, rangePreset }))} onMachineSelect={(machineId) => { setSelectedMachine(machineId); window.history.pushState({}, '', `/machine-detail?machineId=${machineId}`); }} /> : <ErrorPanel message="No API data available." onRetry={() => setNonce((value) => value + 1)} />;
