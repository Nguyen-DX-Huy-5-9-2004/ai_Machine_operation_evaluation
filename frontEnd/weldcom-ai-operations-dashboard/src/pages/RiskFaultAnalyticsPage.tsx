...
export function RiskFaultAnalyticsPage({ filters = { datasetMode: 'historical' } }: { filters?: RuntimeFilters }) {
  ...
  return <section className="space-y-4"><div className="glass-panel p-5"><div className="panel-title">{t('Risk & Fault Analytics')}</div><p className="mt-1 text-sm text-slate-400">{t('Stored policy actions and L2 risk outputs. No policy is recalculated in the browser.')}</p><div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">{data.actionDistribution?.map((row) => <article key={row.level} className="rounded border border-blue-200/15 p-4"><div className="text-xs text-slate-400">{t(row.level)}</div><strong className="text-2xl">{row.count.toLocaleString()}</strong></article>)}</div></div><div className="glass-panel overflow-x-auto p-5"><table className="data-table"><thead><tr><th>{t('Machine')}</th><th>{t('Fault 30 min')}</th><th>{t('Maintenance 30 events')}</th><th>{t('Repair 30 events')}</th></tr></thead><tbody>{data.riskWindows.map((row) => <tr key={row.machine_id}><td>{row.machine_id}</td><td>{t(show(row.risk_fault_30min))}</td><td>{t(show(row.risk_maintenance_30_events))}</td><td>{t(show(row.risk_repair_30_events))}</td></tr>)}</tbody></table></div></section>;
}
