...
export function RuntimeMachineDetailPage({ machineId, filters, onBack }: { machineId: number; filters: RuntimeFilters; onBack: () => void }) {
  ...
  return <div className="space-y-4">
    ...
    <section className="glass-panel p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div><div className="text-2xl font-black">{display}</div><div className="mt-1 text-sm text-slate-400">{data.meta.datasetMode} · {data.meta.source}</div></div><div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-5"><Value label="Action" value={String(summary.operational_action_level ?? 'UNREADY')} /><Value label="Overall risk" value={formatRisk(toNumber(summary.operational_overall_risk_score))} /><Value label="Latest event" value={formatDate(summary.event_start_time)} /><Value label="L1 readiness" value={friendlyReadiness(readiness)} title={readiness} /><Value label="Location / group" value={`${summary.location_id ?? '—'} / ${summary.machine_group_id ?? '—'}`} /></div></div></section>
...
function Value({ label, value, title }: { label: string; value: string; title?: string }) { return <div title={title}><div className="text-xs uppercase text-slate-500">{label}</div><div className="mt-1 font-bold text-white">{value}</div></div>; }
...
function badge(value: unknown) { return <span className="chip">{String(value ?? 'UNREADY')}</span>; }
function Timeline({ rows }: { rows: Array<Record<string, unknown>> }) { return <section className="glass-panel p-5"><div className="panel-title">Operational Timeline</div><div className="mt-4 space-y-3">{rows.slice(0, 12).map((row, index) => <div key={String(row.event_uid ?? index)} className="flex items-center justify-between border-b border-blue-200/10 pb-3 text-sm"><div><strong>{formatDate(row.event_start_time ?? row.eventTime)}</strong><div className="text-slate-400">Status {String(row.status_id ?? '—')} · {String(row.location_id ?? 'Location unavailable')}</div></div>{badge(row.operational_action_level)}</div>)}</div></section>; }
function EventTable({ rows }: { rows: Array<Record<string, unknown>> }) { return <section className="glass-panel overflow-x-auto p-4">{rows.length === 0 ? <div className="p-8 text-center text-slate-400">Không có dữ liệu trong phạm vi đã chọn.</div> : <table className="data-table"><thead><tr><th>Event</th><th>Time</th><th>Status</th><th>Duration</th><th>Action</th><th>Readiness</th><th>Reason</th></tr></thead><tbody>{rows.map((row, index) => { const reason=String(row.readiness_reason ?? 'Unavailable'); return <tr key={String(row.event_uid ?? index)}><td title={String(row.event_uid ?? '')}><strong>{String(row.event_id ?? '—')}</strong><div className="text-xs text-slate-500">{String(row.event_uid ?? '').slice(0, 28)}</div></td><td>{formatDate(row.event_start_time ?? row.eventTime)}</td><td>{String(row.status_id ?? '—')}</td><td>{row.duration_sec == null ? '—' : `${Math.round(Number(row.duration_sec) / 60)} min`}</td><td>{badge(row.operational_action_level)}</td><td title={reason}>{friendlyReadiness(reason)}</td><td title={String(row.final_reason_v2 ?? '')}>{String(row.final_reason_v2 ?? '—').slice(0, 72)}</td></tr>; })}</tbody></table>}</section>; }
