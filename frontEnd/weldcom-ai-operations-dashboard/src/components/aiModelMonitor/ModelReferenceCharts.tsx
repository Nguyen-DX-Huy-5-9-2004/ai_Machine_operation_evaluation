...
function ChartTooltip({ active, payload, label, coordinate, config }: { active?: boolean; payload?: Array<{ name?: string; value?: number; color?: string; payload?: Record<string, string | number> }>; label?: string; coordinate?: { x?: number; y?: number }; config: MonitorChartSeriesConfig[] }) {
  ...
  return <FloatingChartTooltip active={active} coordinate={coordinate}><strong>{formatChartTime(label, language)}</strong>{payload.map((item) => {
    const series = config.find((candidate) => t(candidate.label) === item.name);
  ...
  })}{typeof profile === 'string' ? <em>{t('Profile')}: {profile}</em> : null}</FloatingChartTooltip>;
}
...
function RenderChart({ series, title, source, seriesConfig = [] }: { series: Array<Record<string, string | number>>; title: string; source: MonitorProvenance; seriesConfig?: MonitorChartSeriesConfig[] }) {
  ...
  const common = <><CartesianGrid stroke="rgba(91,147,204,.12)" vertical={false} /><XAxis dataKey={xKey} interval={rotated ? 0 : 'preserveStartEnd'} angle={rotated ? -24 : 0} textAnchor={rotated ? 'end' : 'middle'} height={rotated ? 48 : 28} tickFormatter={(value) => formatChartTime(value, language)} tick={{ fill: '#829bb5', fontSize: 10 }} tickLine={false} axisLine={false} minTickGap={8} /><Tooltip content={<ChartTooltip config={config} />} cursor={false} allowEscapeViewBox={{ x: true, y: true }} /><Legend wrapperStyle={{ fontSize: 10, paddingTop: 5 }} /></>;
  const thresholdDomain = title.includes('Threshold Stability') ? ['dataMin - 0.01', 'dataMax + 0.01'] : ['auto', 'auto'];
  const regular = useBars ? <BarChart data={series} margin={{ top: 8, right: 10, left: -14, bottom: 2 }}>{common}<YAxis tick={{ fill: '#829bb5', fontSize: 10 }} tickLine={false} axisLine={false} unit={config.some((item) => item.unit === 'percent_0_100') ? '%' : undefined} />{config.map((item, index) => <Bar key={item.key} name={t(item.label)} dataKey={item.key} fill={colors[index % colors.length]} radius={[3, 3, 0, 0]} />)}</BarChart> : <LineChart data={series} margin={{ top: 8, right: 10, left: -14, bottom: 2 }}>{common}<YAxis domain={thresholdDomain} tick={{ fill: '#829bb5', fontSize: 10 }} tickLine={false} axisLine={false} />{config.map((item, index) => <Line key={item.key} name={t(item.label)} type="monotone" dataKey={item.key} stroke={colors[index % colors.length]} strokeDasharray={title.includes('Threshold Stability') && index === 1 ? '5 4' : undefined} strokeWidth={2.1} dot={false} activeDot={{ r: 5, stroke: '#fff', strokeWidth: 2 }} />)}</LineChart>;
  const runHealth = title.includes('Run Success') ? <ComposedChart data={series} margin={{ top: 8, right: 20, left: -14, bottom: 2 }}>{common}<YAxis yAxisId="left" domain={[0, 1]} allowDecimals={false} tick={{ fill: '#829bb5', fontSize: 10 }} /><YAxis yAxisId="right" orientation="right" tick={{ fill: '#829bb5', fontSize: 10 }} /><Bar yAxisId="left" name={t('Success')} dataKey="success" fill="#15d8a5" radius={[3, 3, 0, 0]} /><Bar yAxisId="left" name={t('Failed')} dataKey="failed" fill="#ff3e52" radius={[3, 3, 0, 0]} /><Line yAxisId="right" name={t('Duration (s)')} type="monotone" dataKey="durationSec" stroke="#ffb21a" strokeWidth={2.1} /></ComposedChart> : null;
  const feature = title.includes('Feature Availability') ? <LineChart data={series} margin={{ top: 8, right: 20, left: -14, bottom: 2 }}>{common}<YAxis yAxisId="left" domain={[95, 100]} tick={{ fill: '#829bb5', fontSize: 10 }} /><YAxis yAxisId="right" orientation="right" domain={[0, 5]} tick={{ fill: '#829bb5', fontSize: 10 }} /><Line yAxisId="left" name={t('Feature Availability')} type="monotone" dataKey="featureAvailability" stroke="#15d8a5" strokeWidth={2.1} dot={false} /><Line yAxisId="left" name={t('Event Alignment')} type="monotone" dataKey="eventAlignment" stroke="#2b8cff" strokeWidth={2.1} dot={false} /><Line yAxisId="right" name={t('Missing Rate')} type="monotone" dataKey="missingRate" stroke="#ffb21a" strokeWidth={2.1} dot={false} /></LineChart> : null;
  return <Panel title={t(title)} tooltip={`${source.sourceLabel}. ${source.tooltip}`} source={source} className="amm-reference-chart"><div className="amm-reference-chart__body"><ResponsiveContainer width="100%" height="100%">{runHealth ?? feature ?? regular}</ResponsiveContainer></div>{title.includes('Threshold Stability') ? <p className="amm-chart-note">{t('Demo stability visualization; production thresholds are listed separately.')}</p> : null}</Panel>;
}
