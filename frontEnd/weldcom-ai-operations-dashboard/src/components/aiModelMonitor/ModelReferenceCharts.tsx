...
function RenderChart({ series, title, source, seriesConfig = [] }: { series: Array<Record<string, string | number>>; title: string; source: MonitorProvenance; seriesConfig?: MonitorChartSeriesConfig[] }) {
  ...
  return <Panel title={title} tooltip={`${source.sourceLabel}. ${source.tooltip}`} source={source} className="amm-reference-chart"><div className="amm-reference-chart__body"><ResponsiveContainer width="100%" height="100%">{runHealth ?? feature ?? regular}</ResponsiveContainer></div>{title.includes('Threshold Stability') ? <p className="amm-chart-note">{t('Demo stability visualization; production thresholds are listed separately.')}</p> : null}</Panel>;
}
