...
export function AIModelMonitorPresentation({ data, filters, loading, error, onFilterChange, onRefresh }: { data: AIModelMonitorPayload | null; filters: MonitorFilterState; loading: boolean; error: string | null; onFilterChange: (patch: Partial<MonitorFilterState>) => void; onRefresh: () => void }) {
  const t = useUiText();
  if (error) return <div className="amm-page"><div className="amm-fallback-banner"><FileWarning size={16} /><span>{t(runtimeConfig.isMockMode ? 'Unable to load mock demo data.' : 'Unable to load real API data.')} {t(error)}</span><button type="button" onClick={onRefresh}>{t('Retry')}</button></div></div>;
  ...
  return <div className="amm-page">
    ...
    <section className="amm-kpi-grid" aria-label={t('AI monitor key metrics')}>{data.kpis.map((item) => <MonitorKpiCard key={item.id} item={item} />)}</section>
