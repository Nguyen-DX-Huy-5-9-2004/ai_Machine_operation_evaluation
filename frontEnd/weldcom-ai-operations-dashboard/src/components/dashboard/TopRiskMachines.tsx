  ...
  return (
    <section className={`glass-panel panel-primary top-risk-machines-card p-5${compact ? ' top-risk-machines-card--compact' : ''}`}>
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="panel-title metric-title-with-info">{t('Top Machines by Risk')}<DashboardInfoTooltip text="Ranks machines using the selected operational or data-quality measure. Use machine detail to inspect the event evidence behind a rank." /></div>
        ...
        {sorted.map((machine) => {
          ...
          return (
            <button type="button" key={machine.machineId} className={`grid w-full items-center gap-3 text-left leading-none disabled:cursor-default${compact ? ' grid-cols-[86px_1fr_34px] text-[12px]' : ' grid-cols-[116px_1fr_38px] text-sm'}`} title={`${machine.machineName} | ID ${machine.machineId} | ${machine.locationName}`} disabled={!onSelect || !Number.isInteger(Number(machine.machineId))} onClick={() => onSelect?.(Number(machine.machineId))}>
              <span className="min-w-0">{compact ? <b className="block truncate font-semibold text-slate-200">{machine.machineId}</b> : <><b className="block truncate font-semibold text-slate-200">{machine.machineName}</b><small className="block pt-1 text-[10px] text-slate-500">{t('Machine ID')} {machine.machineId}</small></>}</span>
