...
function timeTick(value: string, granularity: Granularity, language: 'en' | 'vi') {
  const parsed = new Date(value);
  if (Number.isNaN(parsed.getTime())) return value;
  return granularity === 'hour'
    ? parsed.toLocaleString(language === 'vi' ? 'vi-VN' : 'en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
    : parsed.toLocaleDateString(language === 'vi' ? 'vi-VN' : 'en-US', { month: 'short', day: 'numeric' });
}
  ...
  return (
    <section className="glass-panel panel-secondary quality-trend-card p-4">
      <div className="mb-3 flex items-center justify-between">
        <div className="panel-title metric-title-with-info">{t('Quality Issue Trend')}<DashboardInfoTooltip text="Daily distribution of quality outcomes. CHECK_DATA and CHECK_ENERGY indicate data validation needs; they are not machine-fault classifications." /></div>
      ...
      <div className="quality-trend-chart">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 5, right: 10, left: -16, bottom: 22 }} barCategoryGap="24%">
            ...
            <XAxis dataKey="label" tickFormatter={(value) => timeTick(String(value), granularity, language)} minTickGap={granularity === 'hour' ? 80 : 48} tick={{ fill: '#98b3d1', fontSize: 11 }} axisLine={false} tickLine={false} />
            ...
            <Bar dataKey="qualityOk" name={t('QUALITY_OK')} stackId="a" fill="#19f5a5" barSize={18} activeBar={false} />
            <Bar dataKey="checkDataAndEnergy" name={t('CHECK_DATA_AND_ENERGY')} stackId="a" fill="#bd72ff" barSize={18} activeBar={false} />
            <Bar dataKey="checkEnergy" name={t('CHECK_ENERGY')} stackId="a" fill="#ffad1f" barSize={18} activeBar={false} />
            <Bar dataKey="checkData" name={t('CHECK_DATA')} stackId="a" fill="#ff3d5a" radius={[4, 4, 0, 0]} barSize={18} activeBar={false} />
