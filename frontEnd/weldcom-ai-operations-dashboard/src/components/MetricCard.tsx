...
import { translateInfoTooltip, useAppLanguage, useUiText } from '../i18n/appTranslations';
  ...
  return (
    <div className="metric-card" style={{ '--accent': accent, borderColor: `${accent}55` } as CSSProperties}>
      <div className="relative z-10 flex justify-between gap-3">
        <div>
          <div className="metric-title metric-title-with-info" title={metric.note ? translateInfoTooltip(metric.note, language) : undefined}>{t(metric.title)}<DashboardInfoTooltip text={kpiExplanation[metric.id] ?? metric.note ?? metric.title} /></div>
          ...
          <div className="metric-trend" title={`${t('Future source')}: ${metric.sourceField}`}>
